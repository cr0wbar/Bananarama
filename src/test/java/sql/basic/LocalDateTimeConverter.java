...
public class LocalDateTimeConverter implements SqlTypeConverter<LocalDateTime>{
    ...
    @Override
    public LocalDateTime read(ResultSet rs, int index) throws SQLException {
        Timestamp ts;
        return (ts = rs.getTimestamp(index)) == null ? null : ts.toLocalDateTime();
    }

    @Override
    public void write(PreparedStatement ps, int index, LocalDateTime obj) throws SQLException {
        if(obj == null)
